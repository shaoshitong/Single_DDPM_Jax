...
from dpm_solver_jax import DPM_Solver, NoiseScheduleVP, model_wrapper
from models import utils as mutils
from models.utils import (from_flattened_numpy, get_model_fn, get_score_fn,
                          to_flattened_numpy)
...
def get_sampling_fn(config, sde, model, shape, inverse_scaler, eps):
    ...
    if sampler_name.lower() == 'ode':
        ...
        sampling_fn = get_obe_dpmsolver_sampler(sde=sde,
                                                model=model,
                                                shape=shape,
                                                inverse_scaler=inverse_scaler,
                                                denoise=config.sampling.noise_removal,
...
def get_obe_dpmsolver_sampler(sde, model, shape, inverse_scaler,
                              denoise=False, rtol=1e-5, atol=1e-5, method='RK45', eps=1e-3):
    """Probability flow ODE sampler with DPM Solver/DFM Solver++.
    ...
    warpper_vpsde = NoiseScheduleVP(betas=sde.discrete_betas)

    def get_new_model_fn(state):
        old_model_fn = get_model_fn(model, state.params_ema, state.model_state, train=False)
        model_fn = model_wrapper(lambda *args: old_model_fn(*args)[0], warpper_vpsde, model_type="noise")
        return model_fn

    def dpm_solver(prng, pstate, z=None, **kwargs):
        assert "steps" in kwargs, "step shoulb be input!"
        # Initial sample
        rng = flax.jax_utils.unreplicate(prng)
        rng, step_rng = random.split(rng)
