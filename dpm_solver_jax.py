...
class NoiseScheduleVP:
    ...
    def inverse_lambda(self, lamb):
        ...
        if self.schedule == 'linear':
            ...
            t = interpolate_fn(log_alpha.reshape((-1, 1)), jnp.flip(self.log_alpha_array, [1]),
                               jnp.flip(self.t_array, [1]))
            return t.reshape((-1,))
        else:
            log_alpha = -0.5 * jnp.logaddexp(-2. * lamb, jnp.zeros((1,)))
            t_fn = lambda log_alpha_t: jnp.arccos(jnp.exp(log_alpha_t + self.cosine_log_alpha_0)) * 2. * (
                    1. + self.cosine_s) / math.pi - self.cosine_s
            t = t_fn(log_alpha)
            return t
...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None, rng=None):
        if t_continuous.reshape((-1,)).shape[0] == 1:
            t_continuous = t_continuous.tile((x.shape[0]))
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model(x, t_input, rng, **model_kwargs)
        else:
            output = model(x, t_input, cond, rng, **model_kwargs)
        if model_type == "noise":
            return output
    ...
    def model_fn(x, t_continuous, rng=None):
        """
        ...
        if guidance_type == "uncond":
            return noise_pred_fn(x, t_continuous, rng=rng)
